...

{
    ...
    int noChildren(BTNode *node){
        return (node->left == NULL && node->right == NULL) ? 1 : 0;
    }

    if (tree1 == NULL && tree2 == NULL)  // identical if both NULL
        return 1;
    else if (tree1->item != tree2->item)  // items are different
        return 0;
    else if (noChildren(tree1) + noChildren(tree2) == 1)  // only one has children
        return 0;
    else
